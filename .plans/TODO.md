...
- [ ] Deepen the engine boundary so `scan -> plan` and `apply plan` live in
...
- [ ] Add policy-driven selection controls like `--include-dangerous`,
      `--select`, and risk filters on top of the current plan model
